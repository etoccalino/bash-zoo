...
##############################################
url="etoccalino.com"
webroot='/home/etoccalino/webapps/ghost_blog_static'
path="${webroot}/sitemap.xml"
user='etoccalino'	# web server user
group='etoccalino'	# web server group
...
SITEMAPCHECK=$(grep 'Sitemap:' ${webroot}/robots.txt)
	...
	echo "Sitemap: http://${url}/static/sitemap.xml" >> ${webroot}/robots.txt
	echo "Disallow: /ghost/" >> ${webroot}/robots.txt
	fi
fi
##############################################
echo "" > $path
...
if [[ "$debug" = [nN] ]]; then
	wget  -q --delete-after http://www.google.com/webmasters/tools/ping?sitemap=http%3A%2F%2F${url}%2Fstatic%2Fsitemap.xml

	rm -rf ${url}
else
	echo "wget  -q --delete-after http://www.google.com/webmasters/tools/ping?sitemap=http%3A%2F%2F${url}%2Fstatic%2Fsitemap.xml"
